    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="page-header">
                ...
                <small class="text-info" v-if="totalToPay == 0">Total recebido: {{ total | currency 'R$ ' }}</small><br v-if="overdue > 0">
                <small class="text-danger" v-if="overdue > 0"><strong>Credores: {{ overdue | currency 'R$ ' }}</strong></small><br v-if="totalToPay > 0">
                <small v-if="totalToPay > 0"><strong>Total a receber: {{ totalToPay | currency 'R$ ' }}</strong></small>
                ...
                <tr v-for="(idx, bill) in bills" track-by="$index" :class="{'bg-danger': isOverDue(bill)}"
                    ...
                    <td>
                        ...
                        <a href="#" @click.prevent="payBill(idx)" class="btn btn-success btn-xs"
                           v-show="bill.done == 0"><i class="fa fa-dollar fa-fw"></i> Receber</a>
        ...
        computed: {
            ...
            status() {
                ...
                switch (count) {
                    ...
                    case 1:
                        return "Há uma conta a receber";
                    default:
                        return "Há " + count + " contas a serem recebidas";
                }
            },
        },
        methods: {
            payBill(idx) {
                ...
                BillStore.pay({ id: bill.id }, bill).then(function(response) {
            ...
            unpayBill(idx) {
                ...
                BillStore.unpay({ id: bill.id }, bill).then(function(response) {
                    vm.$dispatch('hide-please-wait');
                    bill.done = 0;
                }).catch(function(response) {
                    vm.$dispatch('hide-please-wait');
            ...
            removeBill(bill, modalId) {
                var vm = this;
                BillStore.delete({ id: bill.id}).then(function(response) {
            ...
            newBill() {
                this.$router.go({name: 'contas.receber.nova'});
            }
        },
        events: {
